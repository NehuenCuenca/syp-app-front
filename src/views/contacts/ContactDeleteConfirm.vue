<!-- ContactDeleteConfirm.vue -->
<!-- Confirmación de eliminación de un contacto -->
<template>
  <div class="contact-delete-confirm">
...
const handleDelete = async() => {
    const deletedContact = await deleteItem(props.recordData.id)
    if(!error.value && deletedContact){
      emit('finish', deletedContact);
    } else {
      console.error('Error al eliminar el contacto:', error.value);
      // toast.add({ severity: 'error', closable: true, summary: 'Error al eliminar el contacto:'  + props.recordData.name });  
      return
    }
};
